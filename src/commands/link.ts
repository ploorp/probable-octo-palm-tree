...
export default async function link(msg: PrivmsgMessage, args: string[]) {
  ...
  if (args[0] === 'link') {
    ...
    if (args[1].toLowerCase() === 'lastfm' || args[1].toLowerCase() === 'letterboxd') {
      linkAccount(msg.senderUserID, msg.senderUsername, args[1].toLowerCase(), args[2].toLowerCase());
    ...
    if (args[1].toLowerCase() === 'lastfm' || args[1].toLowerCase() === 'letterboxd') {
      unlinkAccount(msg.senderUserID, msg.senderUsername, args[1].toLowerCase());
