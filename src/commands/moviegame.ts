...
function runInChannelQueue<T>(channelId: string, fn: () => Promise<T>): Promise<T> {
  const prev = channelQueues.get(channelId) ?? Promise.resolve();
  const res = prev.then(() => fn(), () => fn());
  channelQueues.set(channelId, res.catch(() => {}));
  return res;
}
...
async function createInitialState(): Promise<GameState | null> {
  ...
  return { left, right, streak: 0, roundId: nextRoundId++ };
}
...
export default async function moviegame(msg: PrivmsgMessage, args: string[]) {
  ...
  const initialTargetState = activeGames.get(channelId);
  const targetRoundId = initialTargetState?.roundId;

  return runInChannelQueue(channelId, async () => {
    ...
    if (!state) {
      if (choice && targetRoundId !== undefined) {
        return;
      }
    ...
    if (targetRoundId === undefined || state.roundId !== targetRoundId) {
      return;
    }
